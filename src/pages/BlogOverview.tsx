...
const blogPosts = [
  ...
  },
  {
    ...
    desc: "The red flags, like skin changes or persistent pain, that indicate it is time for a professional evaluation.",
          ...
          <div className="mx-auto flex max-w-[900px] flex-col gap-5 px-0">
            {filtered.map((item, index) => (
              <Link
                key={index}
                href={item.href}
                className="group grid overflow-hidden rounded-xl border border-primary/35 bg-white transition-all duration-300 hover:-translate-y-0.5 hover:border-primary hover:shadow-[0_10px_28px_rgba(44,132,127,0.10)] sm:grid-cols-[190px_1fr]"
              >
